              ...
              describe('vb6', function() {
                    it('commentlines2', function() { 
						console.log('Application_name : vb6, Scenario_name : commentlines2');
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\commentlines2";
...
browser.wait(until.presenceOf(element(by.xpath(` //div[@id="canvasCode"]`))), delay, 'Element taking too long to appear in the DOM');
browser.wait(until.elementToBeClickable(element(by.xpath(` //div[@id="canvasCode"]`))), delay);
element(by.xpath(` //div[@id="canvasCode"]`)).getText().then(function(text){  
var description ='';  
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(` //div[@id="canvasCode"]`, text);
    }
else
    {
    Get_Text.set(` //div[@id="canvasCode"]`, text[0]);
    }  
});
element.all(by.xpath(` //div[@id="canvasCode"]`)).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//div[@id="canvasCode"]`, text);
    }
else
    {
    Get_Attribute.set(` //div[@id="canvasCode"]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\commentlines2";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element(by.xpath(` //div[@id="canvasCode"]`)).click();
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\commentlines2";
                        ...
                        stream1.end();});
element(by.xpath(` //div[@id="canvasCode"]`)).click();
element(by.xpath(`/html/body/ngb-modal-window/div/div/div[2]/div`)).isPresent().then(function (text) {
console.log("presence");
if (text == true){
console.log('Assertion Passed: Expected element xpath /html/body/ngb-modal-window/div/div/div[2]/div is present in current page. :Assertion Passed');
}else{
console.log(`Assertion Failed: Expected element xpath/html/body/ngb-modal-window/div/div/div[2]/div is not present in current page. :Assertion Failed`);
browser.quit();
}
});
