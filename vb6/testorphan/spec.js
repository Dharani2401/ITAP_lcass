              ...
              describe('vb6', function() {
                    it('testorphan', function() { 
						console.log('Application_name : vb6, Scenario_name : testorphan');
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\testorphan";
...
browser.wait(until.presenceOf(element.all(by.xpath(`//div`)).get(44)), delay, 'Element taking too long to appear in the DOM');

browser.wait(until.elementToBeClickable(element.all(by.xpath(`//div`)).get(44)), delay);
element.all(by.xpath(`//div`)).get(44).getText().then(function(text){
var description =''; 
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(`//div[44]`, text);
    }
else
    {
    Get_Text.set(`//div[44]`, text[0]);
    }  
});
element.all(by.xpath(`//div`)).get(44).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//div[44]`, text);
    }
else
    {
    Get_Attribute.set(`//div[44]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\testorphan";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element.all(by.xpath(`//div`)).get(44).click();

var assert='';
element.all(by.xpath(`/html/body/app-root/app-layout/section/app-dashboard/div[1]/div[4]/div[1]/a/app-stat/div/div/div/div[2]/div[2]`)).getText().then(function (text) {
if( typeof text ==='object')
    {
    assert=text[0];
    }
...
if (assert == `Orphan/Driver Components`){
console.log('Assertion Passed: Expected', assert, `for the Xpath/html/body/app-root/app-layout/section/app-dashboard/div[1]/div[4]/div[1]/a/app-stat/div/div/div/div[2]/div[2] is equal toOrphan/Driver Components. :Assertion Passed` );
}else{
