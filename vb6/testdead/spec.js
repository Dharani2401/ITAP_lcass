              ...
              describe('vb6', function() {
                    it('testdead', function() { 
						console.log('Application_name : vb6, Scenario_name : testdead');
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\testdead";
...
browser.wait(until.presenceOf(element.all(by.xpath(`//div`)).get(58)), delay, 'Element taking too long to appear in the DOM');

browser.wait(until.elementToBeClickable(element.all(by.xpath(`//div`)).get(58)), delay);
element.all(by.xpath(`//div`)).get(58).getText().then(function(text){
var description =''; 
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(`//div[58]`, text);
    }
else
    {
    Get_Text.set(`//div[58]`, text[0]);
    }  
});
element.all(by.xpath(`//div`)).get(58).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//div[58]`, text);
    }
else
    {
    Get_Attribute.set(`//div[58]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\testdead";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element.all(by.xpath(`//div`)).get(58).click();

browser.wait(until.presenceOf(element.all(by.xpath(`/html/body/app-root/app-layout/section/app-dashboard/div[1]/div[4]/div[3]/a/app-stat/div/div/div/div[2]/div[2]`))), delay, 'Element taking too long to appear in the DOM');
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\testdead";
                        var fname="page5.png"; var stream1 = fs.createWriteStream(path.join(dir, fname));
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element.all(by.xpath(`/html/body/app-root/app-layout/section/app-dashboard/div[1]/div[4]/div[3]/a/app-stat/div/div/div/div[2]/div[2]`)).click();
