              ...
              describe('vb6', function() {
                    it('missingreport', function() { 
						console.log('Application_name : vb6, Scenario_name : missingreport');
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\missingreport";
...
browser.wait(until.presenceOf(element.all(by.xpath(`//div`)).get(49)), delay, 'Element taking too long to appear in the DOM');

browser.wait(until.elementToBeClickable(element.all(by.xpath(`//div`)).get(49)), delay);
element.all(by.xpath(`//div`)).get(49).getText().then(function(text){
var description =''; 
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(`//div[49]`, text);
    }
else
    {
    Get_Text.set(`//div[49]`, text[0]);
    }  
});
element.all(by.xpath(`//div`)).get(49).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//div[49]`, text);
    }
else
    {
    Get_Attribute.set(`//div[49]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\missingreport";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element.all(by.xpath(`//div`)).get(49).click();
...
browser.wait(until.presenceOf(element.all(by.xpath(`//div`)).get(29)), delay, 'Element taking too long to appear in the DOM');

browser.wait(until.elementToBeClickable(element.all(by.xpath(`//div`)).get(29)), delay);
element.all(by.xpath(`//div`)).get(29).getText().then(function(text){
var description =''; 
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(`//div[29]`, text);
    }
else
    {
    Get_Text.set(`//div[29]`, text[0]);
    }  
});
element.all(by.xpath(`//div`)).get(29).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//div[29]`, text);
    }
else
    {
    Get_Attribute.set(`//div[29]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\vb6\\missingreport";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element.all(by.xpath(`//div`)).get(29).click();
var assert='';
...
if (assert.indexOf( `1`)!== -1){
console.log('Assertion Passed: Expected', assert, `for the Xpath/html/body/app-root/app-layout/section/app-missingcompapp/div[3]/div/div/div/div[4] contain 1. :Assertion Passed` );
}else{
console.log('Assertion Failed: Expected', assert, `for the Xpath/html/body/app-root/app-layout/section/app-missingcompapp/div[3]/div/div/div/div[4] But has 1. :Assertion Failed`);
browser.quit();
}
});
});
