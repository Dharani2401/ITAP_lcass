              ...
              describe('PLSQL', function() {
                    it('MasterInventorySearchCheck', function() {
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\PLSQL\\MasterInventorySearchCheck";
...
browser.wait(until.presenceOf(element.all(by.xpath(`//div`)).get(15)), delay, 'Element taking too long to appear in the DOM');

browser.wait(until.elementToBeClickable(element.all(by.xpath(`//div`)).get(15)), delay);
element.all(by.xpath(`//div`)).get(15).getText().then(function(text){
var description =''; 
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(`//div[15]`, text);
    }
else
    {
    Get_Text.set(`//div[15]`, text[0]);
    }  
});
element.all(by.xpath(`//div`)).get(15).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//div[15]`, text);
    }
else
    {
    Get_Attribute.set(`//div[15]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\PLSQL\\MasterInventorySearchCheck";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element.all(by.xpath(`//div`)).get(15).click();
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\PLSQL\\MasterInventorySearchCheck";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element(by.xpath(` //input`)).sendKeys(`Function`);
...
element(by.xpath(`//input`)).isPresent().then(function (text) {
console.log("presence");
var assert='';
if (text == true){
console.log('xpath in same page');
element(by.xpath(`//input`)).getAttribute('value').then(function (text){
if( typeof text ==='object')
    {
    assert=text[0];
    }
...
if(`getAttribute('value')`=="getText()"){
assert=Get_Text.get(`//input`)  }
if(`getAttribute('value')`=="getAttribute('value')"){
assert=Get_Attribute.get(`//input`)  }
...
element(by.xpath(`/html/body/app-root/app-layout/section/app-masterinvapp/div[4]/div/div/div/div[3]/div[2]/table/tbody/tr[1]/td[2]`)).getText().then(function (text) {
var assert1='';
if( typeof text ==='object')
    {
    assert1=text[0];
...
console.log('Assertion Passed: Expected', assert1, `for the Xpath/html/body/app-root/app-layout/section/app-masterinvapp/div[4]/div/div/div/div[3]/div[2]/table/tbody/tr[1]/td[2] contain `, assert, '. :Assertion Passed');
}else{
console.log('Assertion Failed: Expected', assert1, `for the Xpath/html/body/app-root/app-layout/section/app-masterinvapp/div[4]/div/div/div/div[3]/div[2]/table/tbody/tr[1]/td[2] But has`, assert , '. :Assertion Failed');
browser.quit();
}
});
});
