              ...
              describe('AS400', function() {
                    it('MissingComponentReportTotalEntriesRecord', function() { 
						console.log('Application_name : AS400, Scenario_name : MissingComponentReportTotalEntriesRecord');
...
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\AS400\\MissingComponentReportTotalEntriesRecord";
...
if (assert == `Showing 1 to 10 of 377 entries`){
console.log('Assertion Passed: Expected', assert, `for the Xpath/html/body/app-root/app-layout/section/app-missingcompapp/div[3]/div/div/div/div[4] is equal toShowing 1 to 10 of 377 entries. :Assertion Passed` );
}else{
console.log('Assertion Failed: Expected', assert, `for the Xpath/html/body/app-root/app-layout/section/app-missingcompapp/div[3]/div/div/div/div[4] But has Showing 1 to 10 of 377 entries. :Assertion Failed`);
browser.quit();
}
});

browser.wait(until.presenceOf(element(by.xpath(` //select[@id="appln"]//option[@value="Unknown"]`))), delay, 'Element taking too long to appear in the DOM');
browser.wait(until.elementToBeClickable(element(by.xpath(` //select[@id="appln"]//option[@value="Unknown"]`))), delay);
element(by.xpath(` //select[@id="appln"]//option[@value="Unknown"]`)).getText().then(function(text){  
var description ='';  
    if( typeof text ==='object')
    { description=text[0];}
    else{ description=text;} 
...
if( typeof text ==='object')
    {
    Get_Text.set(` //select[@id="appln"]//option[@value="Unknown"]`, text);
    }
else
    {
    Get_Text.set(` //select[@id="appln"]//option[@value="Unknown"]`, text[0]);
    }  
});
element.all(by.xpath(` //select[@id="appln"]//option[@value="Unknown"]`)).getAttribute('value').then(function (text) {
if( typeof text ==='object')
    {
    Get_Attribute.set(`//select[@id="appln"]//option[@value="Unknown"]`, text);
    }
else
    {
    Get_Attribute.set(` //select[@id="appln"]//option[@value="Unknown"]`, text[0]);
    }  
});
browser.takeScreenshot().then(function (png) {var dir="D:\\iTAP\\Recorded_Scenarios\\AS400\\MissingComponentReportTotalEntriesRecord";
                        ...
                        stream1.write(new Buffer(png, 'base64'));
                        stream1.end();});
element(by.xpath(` //select[@id="appln"]//option[@value="Unknown"]`)).click();
